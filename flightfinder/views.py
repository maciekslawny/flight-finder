...
from flightfinder.models import Flight, FlightPrice, City
...
class Ticket():
    ...
    def total_price(self):
        return self.flight.get_recent_price() + self.return_flight.get_recent_price()
...
def home(request):
    ...
    if request.method == 'POST':
        ...
        departure_flights = Flight.objects.filter(departure_city__name='Gdansk',
                                                  arrival_city__name='Alicante')
        flights = []
        for flight in departure_flights:
            flight_price = flight.get_recent_price()
            return_flights = Flight.objects.filter(departure_city__name='Alicante',
                                                   arrival_city__name='Gdansk',
                                                   flight_date__gt=flight.flight_date,
                                                   flight_date__lte=flight.flight_date + timedelta(days=6))
            for return_flight in return_flights:
                duration = return_flight.flight_date - flight.flight_date
                ticket = Ticket(flight, return_flight, duration.days, flight.get_weekday(),
                                return_flight.get_weekday())
                flights.append(ticket)
...
class ImportFlightsData():
    ...
    def get_search_url(self, city_departure, city_arrival):
        if city_departure == 'Gdansk' and city_arrival == 'Alicante':
            return 'https://www.google.com/travel/flights/search?tfs=CBwQAhopEgoyMDI0LTEwLTE2KABqDAgDEggvbS8wMzVtNnILCAMSBy9tLzB6YzZAAUgBcAGCAQsI____________AZgBAg&hl=pl'
        if city_departure == 'Alicante' and city_arrival == 'Gdansk':
            return 'https://www.google.com/travel/flights/search?tfs=CBwQAhopEgoyMDI0LTEwLTE2KABqCwgDEgcvbS8wemM2cgwIAxIIL20vMDM1bTZAAUgBcAGCAQsI____________AZgBAg&hl=pl'
...
def update(request):
    ...
    test.import_flights('Gdansk', 'Alicante')
    # chromeOptions = Options()
    # chromeOptions.add_argument("--no-sandbox")
    # chromeOptions.add_argument('--headless')
    # chromeOptions.add_argument('--disable-dev-shm-usage')
    ...
    # chromeOptions.add_argument("--ignore-certificate-errors")
    # chromeOptions.headless = True
    # s = Service("/usr/bin/chromedriver/chromedriver")
    # driver = webdriver.Chrome(options=chromeOptions)
    #
    #
    ...
    # time.sleep(5)
    # # Wydrukowanie zawartości strony
    # print(page_source)
    # time.sleep(5)
